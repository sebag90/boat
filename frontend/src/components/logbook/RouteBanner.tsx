...
export function RouteBanner({ start, goal }: { start: string; goal: string }) {
  ...
  return (
    <section className="flex flex-wrap items-center gap-3 rounded-2xl bg-parchment px-4 py-3.5 ring-1 ring-brass-200">
      <Endpoint
        icon={<Anchor className="size-3.5" />}
        label={t('logbook.start')}
        value={start || '—'}
      />
      <MoveRight className="size-5 shrink-0 text-brass-400" />
      <Endpoint
...
function Endpoint({
  ...
  return (
    <div className="min-w-0 flex-1">
      <p className="flex items-center gap-1.5 text-[0.65rem] font-semibold tracking-[0.12em] text-brass-700 uppercase">
        {icon}
        {label}
      </p>
