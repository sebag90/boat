    ...
    <div className={className}>
      <div
        ...
        className={cn(
          ...
          dragging
            ? 'border-ocean-500 bg-ocean-50 ring-4 ring-ocean-100'
            : 'border-navy-200 bg-white/60 hover:border-ocean-400 hover:bg-ocean-50/50',
        )}
      >
        <UploadCloud className={cn('size-7', dragging ? 'text-ocean-600' : 'text-navy-300')} />
        <p className="text-sm font-semibold text-navy-800">
          {label ?? (multiple ? t('dropzone.title') : t('dropzone.single'))}
        </p>
        {multiple && <p className="text-xs text-navy-400">{t('dropzone.subtitle')}</p>}
        <input
          ref={inputRef}
          type="file"
          hidden
      ...
      {files.length > 0 && (
        <div className="mt-3">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-[0.7rem] font-semibold tracking-[0.08em] text-navy-500 uppercase">
